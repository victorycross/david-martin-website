...
export function AdminStatusTable({ allMembers, rsvps }: AdminStatusTableProps) {
  ...
  const emailInvite = (member: FamilyMember) => {
    ...
    if (member.email) {
      ...
      toast({ title: "Email opened", description: `Composing email to ${member.name} (${member.email})` });
    } else {
      // No email — copy the invite text to clipboard instead
      navigator.clipboard.writeText(`Subject: ${subject}\n\n${body}`);
      toast({
        title: "Invite text copied",
        description: `No email on file for ${member.name}. Invite text copied to clipboard — paste it into your email client.`,
      });
    }
  };
                ...
                <td className="text-right">
                  <div className="flex items-center justify-end gap-1.5">
                    ...
                    <button
                      ...
                      className="reunion-invite-btn"
                      title={`Email invite to ${s.member.name}`}
                    >
                      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
