...
export default function MeditationSection() {
  ...
  const meditationScript = [
    ...
    "We begin.",
    "",
    "Be still, and know that I am God. [pause, two breaths]",
    "Be still, and know that I am. [pause, two breaths]",
    "Be still, and know. [pause, two breaths]",
    "Be still. [pause, two breaths]",
    "Be. [pause, three breaths]",
    "",
    ...
    "Be. [pause, three breaths]",
    "",
    "Now let the words fade.",
    "Rest in the silence.",
    "Feel the stillness within you.",
    "Stay here for a few breaths… and when you are ready, gently open your eyes."
  ];
        ...
        {isReading && (
          <div className="mt-8 bg-white/5 rounded-lg p-6">
            ...
            <div className="space-y-3 text-white/90 leading-relaxed">
              {meditationScript.map((line, index) => (
                <div key={index} className={`${line === "" ? "h-2" : ""}`}>
                  {line && (
                    <p className={`${line.startsWith("Be still") ? "font-medium text-white" : ""} ${line.includes("[pause") ? "italic text-white/70" : ""}`}>
                      {line}
                    </p>
                  )}
                </div>
